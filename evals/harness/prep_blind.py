...
rotation (each config occupies each slot twice across the 6 problems), and emit
rotation_map.json so the aggregator can de-anonymize after judging."""
import json
import shutil
...
# Balanced rotation: positions [A, B, C] -> config, one row per problem id.
ROTATION = {
...
def main():
    ...
    if missing:
        print("MISSING / too-small responses:")
        for m in missing:
            print("  -", m)
    else:
