#!/usr/bin/env python3
"""Build a self-contained HTML report for the 3-way critical-thinking eval:
overall scoreboard + per-problem side-by-side responses with judge scores."""
import html
import json
import re
...
DIM_LABEL = {"rigor": "Rigor", "calibration": "Calibration", "bias_awareness": "Bias awareness",
             "depth": "Depth", "usefulness": "Usefulness"}


def md(text):
    """Minimal, safe markdown -> HTML (headings, bold, italic, inline code, lists)."""
    ...
    for ln in lines:
        ...
        if h:
            close()
            lvl = len(h.group(1))
            out.append(f"<h{lvl+2}>{inline(h.group(2))}</h{lvl+2}>")
            continue
...
def main():
    ...
    summ = verdicts["summary"]
    pp = verdicts["per_problem"]

    # winner by overall mean
    ...
    *{box-sizing:border-box} body{margin:0;font-family:-apple-system,Segoe UI,Roboto,Helvetica,Arial,sans-serif;
    ...
    .probhead{padding:18px 22px;background:#0f172a;color:#fff} .probhead .n{font-size:12px;letter-spacing:.08em;text-transform:uppercase;color:#94a3b8}
    .probhead .q{font-size:15px;margin-top:6px;color:#e2e8f0}
    ...
    .reasons{padding:14px 22px;background:#fbfbfc;border-top:1px solid #eef0f2;font-size:13px;color:#374151}
    .reasons .r{margin:5px 0;padding-left:14px;border-left:3px solid #cbd5e1} .legend{font-size:13px;color:#6b7280;margin-top:6px}
    """

    h = [f"<!doctype html><meta charset='utf-8'><title>Critical-thinking 3-way eval</title><style>{css}</style>"]
    h.append("<div class='wrap'>")
    h.append("<h1>Critical-thinking skill: three-way eval</h1>")
    h.append("<p class='sub'>Claude 4.8 (no skill) vs Claude 4.8 + critical-thinking skill vs GPT-5.5 (xhigh), "
             f"across 6 reasoning problems, blind-judged ({verdicts['n_judge_files']} judge passes, A/B/C rotated).</p>")
    ...
    h.append("<p class='legend'>Each cell shows the mean judge score (0 to 10). Wins = times a judge ranked that "
             "response best. Lower average rank is better (1 = best).</p>")
    h.append("</div>")
    ...
    for p in problems:
        ...
        h.append(f"<div class='probhead'><div class='n'>Problem {p['id']} &middot; {p['name']}</div>"
                 f"<div class='q'>{html.escape(p['prompt'])}</div></div>")
        h.append("<div class='cols'>")
        # order columns base, skill, gpt5 for consistent reading
        for c in CONFIGS:
            m = META[c]
            ...
            h.append(f"<div class='resp'>{md(body)}</div>")
            h.append("</div>")
        h.append("</div>")  # cols
        # reasonings
        reasons = []
